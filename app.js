...
// 2026-08校準:排序改依條件回測5日實際表現(訊號矛盾>價量齊揚>出貨嫌疑>強力關注),
// 不再照分類名稱給人的直覺排列——"強力關注"這個名字聽起來最強,但回測顯示它反而是表現最弱的一組
const CATEGORY_ORDER = ["訊號矛盾", "價量齊揚", "出貨嫌疑", "強力關注"];
...
const state = { index: [], watchlistCodes: new Set(), watchlistSha: undefined, currentDay: null, codeNameMap: {}, chartsData: {}, activeChart: null, allSignals: {}, intradaySignals: {}, v3Data: null, v3ActiveKey: null, positions: [], positionsExtra: {}, positionsSha: undefined, positionStatus: {} };
...
function abbreviateReason(reason) {
  const rules = [
    ...
    [/^營收年增(\d+)%$/, (m) => `營收↑${m[1]}%`],
    [/^營收年減(\d+)%$/, (m) => `營收↓${m[1]}%`],
    [/^RSI翻揚\((\d+)→(\d+)\)$/, (m) => `RSI翻揚${m[1]}→${m[2]}`],
    [/^RSI走弱\((\d+)→(\d+)\)$/, (m) => `RSI走弱${m[1]}→${m[2]}`],
    [/^MACD空頭動能減弱$/, () => `MACD轉弱↗`],
    [/^MACD多頭動能減弱$/, () => `MACD轉弱↘`],
    [/^接近布林下軌支撐\(([\d.]+)%\)$/, (m) => `近布林↓${m[1]}%`],
    [/^接近布林上軌壓力\(([\d.]+)%\)$/, (m) => `近布林↑${m[1]}%`],
  ];
